...
<div class="container">
    <h1 class="text-2xl font-bold text-gray-800 mb-4">Daftar Magang</h1>
    ...
    @endif
    <!-- Tombol Tambah Magang -->
    <a href="{{ route('admin.magangs.create') }}" class="inline-flex items-center bg-green-500 hover:bg-green-600 text-white px-4 py-2 mb-4 rounded transition">
        <img src="{{ asset('images/icon/add.svg') }}" alt="Tambah Magang" class="w-5 h-5 mr-2 icon-white">
        Tambah Magang
    </a>

    <!-- Form Pencarian -->
    <form action="{{ route('admin.magangs.index') }}" method="GET" class="mb-4 flex items-center gap-2">
        <input type="text" name="search" value="{{ request('search') }}" placeholder="Cari magang..." class="px-3 py-2 border rounded-md w-full max-w-xs">
...
<form action="{{ route('admin.magangs.index') }}" method="GET" class="mb-4 flex items-center gap-2">
    ...
    <select name="sort" id="sort" class="px-3 py-2 border rounded-md">
        <option value="ID_Magang" {{ request('sort') === 'ID_Magang' ? 'selected' : '' }}>ID</option>
        ...
        <option value="StatusMagang" {{ request('sort') === 'StatusMagang' ? 'selected' : '' }}>Status</option>
        <option value="TanggalMulaiMagang" {{ request('sort') === 'TanggalMulaiMagang' ? 'selected' : '' }}>Tanggal Mulai</option>
        <option value="TanggalBerakhirMagang" {{ request('sort') === 'TanggalBerakhirMagang' ? 'selected' : '' }}>Tanggal Berakhir</option>
    </select>
    ...
    <!-- Pagination -->
    <div class="mt-4">
        {{ $magangs->links('pagination::tailwind') }}
    </div>
        ...
        <table class="min-w-full divide-y-2 divide-gray-200 bg-white text-sm">
            ...
            <tbody class="divide-y divide-gray-200">
                ...
                <tr class="odd:bg-gray-50">
                    <td class="whitespace-nowrap px-4 py-2 font-medium text-gray-900">{{ $magang->ID_Magang }}</td>
                    <td class="whitespace-nowrap px-4 py-2 text-gray-700">{{ $magang->peserta->Nama }}</td>
                    <td class="whitespace-nowrap px-4 py-2 text-gray-700">{{ $magang->divisi->Divisi }}</td>
                    <td class="whitespace-nowrap px-4 py-2 text-gray-700">{{ $magang->TanggalMulaiMagang }}</td>
                    <td class="whitespace-nowrap px-4 py-2 text-gray-700">{{ $magang->TanggalBerakhirMagang }}</td>
                    <td class="whitespace-nowrap px-4 py-2 text-gray-700">{{ ucfirst($magang->StatusMagang) }}</td>
                    <td class="whitespace-nowrap px-4 py-2 text-gray-700">
                        <!-- Tombol Edit -->
                        <a href="{{ route('admin.magangs.edit', $magang->ID_Magang) }}" class="inline-block transition">
                        ...
                        <!-- Tombol Hapus -->
                        <form action="{{ route('admin.magangs.destroy', $magang->ID_Magang) }}" method="POST" style="display:inline;">
                            @csrf
                            @method('DELETE')
                ...
                @empty
                <tr>
                    <td colspan="7" class="text-center py-4">Tidak ada data magang.</td>
                </tr>
                @endforelse
            </tbody>
        </table>
